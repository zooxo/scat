...
static byte slot, ptr;          // Playstring position
...
static void playstring(void) {  // Intitialize playing string
  ptr = 0;
  isnewnumber = true;
  isplaystring = true;
}
...
static void (*dispatch[])(void) = { // Function pointer array for g and f
  ...
  &do_gauss, &do_annu, &do_mult, &do_nop,
  &do_nop, &do_div, &do_swap, &do_add,
  &do_lit, &do_rot
};
...
void loop() {
  ...
  if (isplaystring) { // ### Plays string (f=? g=>)
    const char *ps[6];
    ...
    ps[0] = "?:>5?6>4";         // YpowerX
    ps[1] = ">1;?6=1?;>7";      // COS
    ps[2] = ">1;;?6=1?;>7>9?6"; // TAN
    ps[3] = ">0=90?;";          // ACOS
    //ps[4] = "10>5?6>4?6";       // Y*10^X
    ps[4] = ";;;?6?6:07?6=?:1:6?6=?6>41?;>9?:;?6=2?9>40:3989423?6"; // GAUSS-CDF-PDF
    ps[5] = "?:;;1?;?:?=>5?6>4>9=1?;?=?=?=?9"; // ANNU

    if (ps[slot][ptr] == NULL) isplaystring = false;
    else key = ps[slot][ptr++] - '/';
